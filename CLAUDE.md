...
- `items`: name, slot (`weapon`/`shield`/`head`/`body` — replaced the old `type` in [ADR-003](.claude/adr/ADR-003-battle-stats-and-equipment-slots.md); `body` is a full worn suit, `shield` is carried and is the one slot with a built-in mobility penalty), description (nullable, pure flavour text — no mechanical effect), faction (nullable — NULL is universal, sold to everyone), strength_delta, defense_delta, speed_delta, dexterity_delta, min_level, cost, image. One equipped item per slot; `MarketService::equip()` is keyed on `slot`. Market listing filters to `faction IS NULL OR faction = character.faction`, and `MarketService::buy()` re-checks it (the item id is client input).
